...
internal class PlayerStats
{
    ...
    public PlayerStats()
    {
        ...
        for (int i = 0; i < 32; i++)
        {
            StagePB[i] = new Dictionary<int, PersonalBest>();
            StagePB[i][0] = new PersonalBest();
        }
    ...
    {
        var player_maptime = await APICall.GET<API_MapTime>($"/surftimer/playerspecificdata?player_id={player.Profile.ID}&map_id={player.CurrMap.ID}&style={style}&type=0");
        ...
        {
            PB[style].Checkpoint[cp.cp] = new Checkpoint(cp.cp, cp.ticks, cp.start_vel_x, cp.start_vel_y, cp.start_vel_z, cp.end_vel_x, cp.end_vel_y, cp.end_vel_z, cp.end_touch, cp.attempts);
        }
    }

    public async void LoadStageTime(Player player, int style = 0)
    {
        var player_maptime = await APICall.GET<API_MapTime[]>($"/surftimer/playerspecificdata?player_id={player.Profile.ID}&map_id={player.CurrMap.ID}&style={style}&type=2");
        if (player_maptime == null)
        {
    ...
    {
        var player_maptime = await APICall.GET<API_MapTime[]>($"/surftimer/playerspecificdata?player_id={player.Profile.ID}&map_id={player.CurrMap.ID}&style={style}&type=1");
        if (player_maptime == null)
        {
